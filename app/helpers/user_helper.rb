module UserHelper
  ...
  def voting_user_info(u, flag)
    if flag > 0 #positive vote
      "#{user_small_avatar_link(u)} #{link_to u, user_path(u)}".html_safe
    else
      "#{link_to u, user_path(u)} #{user_small_avatar_link(u)}".html_safe
    end
  end
end
